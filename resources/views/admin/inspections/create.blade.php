...
@endpush
@section('content')
<div x-data="inspectionForm()" class="max-w-4xl mx-auto">
    <div class="mb-6">
    ...
    <form method="POST" action="{{ route('admin.inspections.store', [$reservation, $type]) }}" enctype="multipart/form-data" id="inspectionForm">
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-3">
                                    <span class="font-medium text-gray-800">{{ $checklistItem->label }}</span>
                                    <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded">{{ $item->equipment->name }}</span>
                                </div>
                                @if($checklistItem->description)
                                    <p class="text-xs text-gray-500 mt-1">{{ $checklistItem->description }}</p>
                                @endif
                            ...
                            <div class="flex gap-2">
                                ...
                                <label class="cursor-pointer">
                                    ...
                                    <span class="px-3 py-1.5 text-xs font-medium rounded-full border-2 border-transparent
                                        ...
                                        bg-gray-50 text-gray-600 hover:bg-gray-100 select-none">
                                        {{ $meta['label'] }}
                                    </span>
                                </label>
                                @endforeach
        ...
        <div class="bg-white rounded-xl shadow-sm border mb-6">
            ...
            <div class="p-6">
                ...
                <p class="text-xs text-gray-400 mt-2">Ajoutez des photos de l'état général du matériel (max 10 Mo par photo)</p>
                <div id="photoPreview" class="mt-4 grid grid-cols-4 gap-3"></div>
            </div>
        </div>
        ...
        @if($type === 'return')
        {{-- Dégradations --}}
        <div class="bg-white rounded-xl shadow-sm border mb-6" x-data="{ charges: [] }">
                ...
                @foreach($reservation->items as $item)
                    ...
                    <div class="border rounded-lg p-3 mb-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-bold text-red-600">{{ number_format($scaleItem->amount, 2, ',', ' ') }} €</span>
                        </div>
            ...
            <div class="p-6">
                <div class="space-y-3 mb-6">
                    <label class="flex items-center gap-3 cursor-pointer p-3 border rounded-lg hover:bg-green-50"
                        ...
                        <input type="radio" name="deposit_action" value="return" x-model="action" class="text-green-500">
                        <div>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer p-3 border rounded-lg hover:bg-red-50"
                        ...
                        <input type="radio" name="deposit_action" value="cash_new" x-model="action" class="text-red-500">
                        <div>
                ...
                <div x-show="action === 'cash_new'" x-cloak class="grid grid-cols-2 gap-4 border-t pt-4">
                    <div>
            ...
            <div class="p-6 grid grid-cols-2 gap-8">
                <div>
                    ...
                    <div class="border-2 border-gray-200 rounded-lg overflow-hidden bg-white">
                        <canvas id="clientSignaturePad" width="350" height="150"></canvas>
                    </div>
                ...
                <div>
                    ...
                    <div class="border-2 border-gray-200 rounded-lg overflow-hidden bg-white">
                        <canvas id="adminSignaturePad" width="350" height="150"></canvas>
                    </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('admin.reservations.show', $reservation) }}" class="px-6 py-2.5 border rounded-lg text-sm text-gray-700 hover:bg-gray-50">
                Annuler
            </a>
...
<script>
const clientPad = new SignaturePad(document.getElementById('clientSignaturePad'), { backgroundColor: 'rgb(255,255,255)' });
const adminPad = new SignaturePad(document.getElementById('adminSignaturePad'), { backgroundColor: 'rgb(255,255,255)' });
...
// Before submit, capture signatures
